...
date: 'Lecture 3, (B) Feb 4, (A) Feb 9'
...
1. Create a scatter plot of `hp` (horsepower) vs `mpg` (miles per gallon).
2. Create a bar chart showing the count of cars by number of gears (`gear`).
...
<mark>**Common Error:**</mark> What happens if you put a fixed value inside `aes()`?
...
[ColorBrewer](https://colorbrewer2.org/) provides color palettes designed for data visualization:
...
1. Create a scatter plot of `Sepal.Length` vs `Sepal.Width`, colored by `Species`.

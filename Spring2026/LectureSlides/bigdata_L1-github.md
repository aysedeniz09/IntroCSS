...
You can create a chunk by typing it manually or by using the **Insert
Code Chunk** button in RStudio.

Code chunks start with `{r} and end with`.

You can also add: - OPTIONAL: a **chunk label** (for identification) -
**chunk options** (to control how code and output appear)

Example:

- Chunk labels are ***OPTIONAL*** and if you do put one it should be
...
You can set options that apply to all code chunks in the document. This
is usually done in the first chunk of the file.

Example, this displays code in the output document, prevents messages
(such as package startup messages) and warnings from appearing in the
output document. :
...
| `error` | `FALSE` | `TRUE` = display error messages in the document; `FALSE` = stop rendering when an error occurs |
| `eval` | `TRUE` | Run code in the chunk |
| `include` | `TRUE` | Include the chunk output in the document after running |
| `message` | `TRUE` | Display messages in the document |
| `warning` | `TRUE` | Display warnings in the document |
| `results` | `"markup"` | `"asis"` = pass results through as-is; `"hide"` = hide results; `"hold"` = show all results after all code |
| `fig.align` | `"default"` | Align figure: `"left"`, `"right"`, or `"center"` |
| `fig.alt` | `NULL` | Alt text for a figure |
| `fig.cap` | `NULL` | Figure caption (character string) |
| `fig.path` | `"figure/"` | Prefix folder for generated figure file paths |
| `fig.width` / `fig.height` | `7` | Plot dimensions in inches |
| `out.width` | *(none)* | Rescale output width (e.g., `"75%"`, `"300px"`) |
| `collapse` | `FALSE` | Collapse source and output into a single block |
| `comment` | `"##"` | Prefix for each line of results |
| `child` | `NULL` | File(s) to knit and include in the document |
| `purl` | `TRUE` | Include/exclude chunk when extracting code with `knitr::purl()` |
